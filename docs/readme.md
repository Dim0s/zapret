как я запускал все эти пельмени:

через автозагрузку (папка в меню пуск в win7) кидал туда ярлык батника с аргументами запуска.
ни когда не юзал батники которые устанавливают этот софт как СЛУЖБУ

иногда при неведомой жопе теряетя определение пути драйвера WINDIVERT
при запуске софт резко закрывается и всё
____________________________________________________________________________
для лечения удалить ключи в реестре:

[HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\services\WinDivert]
[HKEY_LOCAL_MACHINE\SYSTEM\ControlSet002\services\WinDivert]
[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\WinDivert]

искать их тут:

нажать WIN + R и вбить regedit
____________________________________________________________________________

для удаления этих ключей:

[HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Enum\Root\LEGACY_WINDIVERT]
[HKEY_LOCAL_MACHINE\SYSTEM\ControlSet002\Enum\Root\LEGACY_WINDIVERT]

надо сделать:

правый клик на удаляемой строчке => разрешения => дополнительно => владелец

поставить галку => заменить владельца подконтейнеров и объектов

применить и в главном окне всем группам поставить галку Полный доступ => разрешить
____________________________________________________________________________

так как юзаю это дело в виртуалке и после удаления ключей всё заработало, просто забил болт и не стал углубляться в эту жопу в поиске причин (скорей всего запуская другие программы использующие этот же драйвер, к примеру GoodbyeDPI они вносят свои коррективы, меняя пути и конфиги под себя, что ушатает совместимость с первоначально запускаемым софтом, но это не точно)

всё что делаете, всё на свой страх и риск
____________________________________________________________________________
